# niri: scrollable-tiling Wayland compositor. This is the whole desktop
# session -- compositor, greeter command, portals, idle/lock, launcher, wallpaper.
# The generic Wayland bits it shares with sway are in wayland.nix.
{ inputs, ... }:
{
  flake.modules.nixos.niri =
    { pkgs, ... }:
    {
      # The NixOS module installs niri, registers the session, and sets up the
      # xdg portals.
      programs.niri.enable = true;
      # Personal fork. Roll back by removing this line if it causes issues.
      programs.niri.package = inputs.niri-fork.packages.${pkgs.stdenv.hostPlatform.system}.niri;
  ...
  flake.modules.homeManager.niri =
    ...
    in
    {
      ...
      # Home Manager has no `programs.niri` module, so the compositor is
      # configured by writing its KDL config file directly. The compositor itself
      # is enabled system-wide via `programs.niri.enable` above.
      xdg.configFile."niri/config.kdl".text = ''
        // Let outputs be configured at runtime by nwg-displays
        include "monitor.kdl"
        ...
        binds {
            ...
            // queries niri's IPC for the focused PID); see modules/cli/terminal.nix.
